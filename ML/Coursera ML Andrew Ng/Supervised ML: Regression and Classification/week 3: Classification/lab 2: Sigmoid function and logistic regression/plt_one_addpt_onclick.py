...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        # 检查是否有 rectangles 或 patches 属性
        if hasattr(bcid, 'rectangles') and len(bcid.rectangles) > 0:
            # 旧版 Matplotlib 的 CheckButtons
            ...
            if len(bcid.lines) > 0 and len(bcid.lines[0]) > 1:
                bcid.lines[0][0].set_ydata([ymax, ymin])
                bcid.lines[0][1].set_ydata([ymin, ymax])
        elif hasattr(bcid, 'patches') and len(bcid.patches) > 0:
            # 新版 Matplotlib 的 CheckButtons
            ...
            print("Warning: Could not resize checkbox - unsupported CheckButtons version")